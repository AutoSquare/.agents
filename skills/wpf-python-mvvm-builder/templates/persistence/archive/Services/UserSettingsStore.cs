...
/// <summary>
/// 读写 %AppData%/{AppId}/user_settings.json（archive 模式：最近工程路径与窗口尺寸）。
/// </summary>
public static class UserSettingsStore
{
    /// <summary>应用数据目录。</summary>
    public static string AppDataDirectory =>
        Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "{{RootNamespace}}");
    ...
    /// 读取上次打开的工程路径。
    /// </summary>
    /// <returns>路径；不存在时 null。</returns>
    public static string? LoadLastProjectPath()
    {
        var settings = Load();
        if (!settings.TryGetValue("lastProjectPath", out var value))
            return null;
    ...
    public static void SaveLastProjectPath(string path)
    {
        var dict = ToMutableDictionary(Load());
        dict["lastProjectPath"] = path;
        Save(dict);
    }
    ...
    private static Dictionary<string, object?> ToMutableDictionary(IReadOnlyDictionary<string, JsonElement> settings)
    {
        ...
        foreach (var pair in settings)
            dict[pair.Key] = pair.Value.ValueKind == JsonValueKind.String ? pair.Value.GetString() : pair.Value.ToString();
        return dict;
    }
