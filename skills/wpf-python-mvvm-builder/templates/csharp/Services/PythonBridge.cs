...
/// <summary>
/// 与 Python 运行时之间的过桥：子进程、{{WorkspaceEnvVar}} 环境变量、标准输入输出。
/// </summary>
public sealed class PythonBridge
{
    ...
    /// 启动 Python 解释器执行指定脚本。
    /// </summary>
    /// <param name="scriptRelativeToPyFolder">相对于 {{PyFolder}} 的脚本路径。</param>
    /// <param name="workspaceRoot">写入环境变量 {{WorkspaceEnvVar}} 的工作区根路径。</param>
    ...
    private static ProcessStartInfo CreateStartInfo(string scriptPath, string workspaceRoot)
    {
        ...
        psi.Environment["{{WorkspaceEnvVar}}"] = workspaceRoot;
