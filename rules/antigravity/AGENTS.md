...
本文件安装到 `$Antigravity_HOME/AGENTS.md`，适用于本机所�?Antigravity 会话。项目或子目录中�?`AGENTS.md` 可以补充更具体规则；若发生冲突，优先级为：系�?开发者消�?> 项目/子目录规�?> 本全局规则�?
...
- 使用 skills 时，先读取匹�?skill �?`SKILL.md`，再按其流程执行�?

## 按需读取规则

规则文件位于 `$Antigravity_HOME/agent-rules/`。遇到对应任务时，先读取相应文件再执行�?
...
- 创建或修�?Agent Skills：`07-skills-authoring.md`
...
- 项目�?`.agents/` 配置包时：`13-agents-entry.md`

## Skills

全局 skills 位于 `$Antigravity_HOME/skills/`。当用户点名 skill，或任务明显匹配 skill 描述时，必须读取�?skill �?`SKILL.md`；多�?skill 同时适用时，选择最小必要组合并说明使用顺序�?
